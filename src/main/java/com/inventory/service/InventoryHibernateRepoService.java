...
public class InventoryHibernateRepoService implements InventoryRepoService {
	...
	public Product getProductsById(Integer productId) {
		return productRepo.findOne(Long.valueOf(productId));
	}
	...
	public List<Product> getProductsByIds(List<Integer> productIds) {
		...
		queryBuilder.append("select prd from ProductImpl prd where prd.id IN (" + ":prdIds" + ")");
	...
	public List<Product> getProductByFilter(ProductFilter product) {
		Map<String, Object> inParamMap = new HashMap<>();
		StringBuilder queryBuilder = new StringBuilder();
		queryBuilder.append("select prd from ProductImpl prd where 1=1");
